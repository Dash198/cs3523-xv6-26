...
test_pa4_11(void)
{
    ...
    printf("\nDisk Stats (FCFS): Reads=%d, Writes=%d, Avg Latency=%d.%d ticks\n",
           diskstat.disk_reads, diskstat.disk_writes, diskstat.average_latency / 100, diskstat.average_latency % 100);

    diskstat.disk_reads = diskstat.disk_writes = diskstat.average_latency = 0; // reset stats
...
test_pa4_12(void)
{
    ...
    printf("  FCFS stats -> reads=%d writes=%d average_latency=%d.%d ticks\n",
           st.disk_reads, st.disk_writes, st.average_latency / 100, st.average_latency % 100);
    ...
    if (st.disk_reads >= 0 && st.disk_writes >= 0 && st.average_latency >= 0)
